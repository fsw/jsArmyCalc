...
function jsArmyCalc( selector, templateurl ){
	...
	//TODO maybe it would be better to embedd calc html here?	
	var template = $.ajax({
				url: templateurl,
				async: false
	...
	calc.canvas.find('#acMaximize').click(function(){calc.setFullscreen(true)});
	calc.canvas.find('#acMinimize').click(function(){calc.setFullscreen(false)});
	...
	calc.loadInfoXml = function( xml ){
		
		ruleset = {};
		ruleset.version = $(xml).find('ruleset').attr('version');
		
		ruleset.uid = $(xml).find('uid').text();
		ruleset.revision = $(xml).find('revision').text();
		ruleset.name = $(xml).find('name').text();
		ruleset.mediapath = $(xml).find('files').find('media').text();
		
		$(xml).find('files').find('units').find('file').each(function(){
		
			$.ajax({
				calc: calc,
				url: calc.twrurl + $(this).find('path').text(),
				success: calc.calcLoadUnitsXml,
				error: xhrError,
				dataType: 'xml'
			});
		});
